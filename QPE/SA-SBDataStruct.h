...
public:
	...
	enum VOL_MODEL {
		UNKNOW = 0,
		VCP11 = 11,  //降水模式，14层仰角/5
		VCP21 = 21,  //降水模式，9层仰角/6
		VCP31 = 31,  // 晴空模式，5层仰角/10 长脉冲
		VCP32 = 32 //   晴空模式，5层仰角/10 短脉冲
	};
...
public:
	...
	const RadarElevation invalid_RadarElevation;
...
public:
	...
	{
		auto it = elevations.find(el);
		return it == elevations.end() ?  invalid_RadarElevation : it->second;
	}
...
private:
	...
	{
		std::map<VOL_MODEL, std::vector<double> > vcps = {

			{VCP11, {0.50, 1.45, 2.40, 3.35, 4.30, 5.25, 6.20, 7.50, 8.7, 10, 12, 14, 16.7, 19.5} },
			{ VCP21,{ 0.50, 1.45, 2.40, 3.35, 4.30, 6.0, 9.9, 14.6, 19.5 } },
			{ VCP31,{ 0.50, 1.5, 2.5, 3.5, 4.5 } },
			{ VCP31,{ 0.50, 1.5, 2.5, 3.5, 4.5 } }
		};

		std::vector<double> vcp = vcps[ vol_model ];
		for (auto value : vcp)
		{
			if (abs(el - value) < 0.3)
			{
	...
	bool init_elevations()
	{
		...
		for(auto it=alldata.begin(); it != alldata.end(); it++)
		{
			...
			case VOL_BEG:
				...
				vol_model = get_vol_model(it->VcpNumber);
			case ELV_BEG:  //仰角开始
	...
	//获取体扫模式
	VOL_MODEL get_vol_model(unsigned short VcpNumber)
	{
		VOL_MODEL status;
